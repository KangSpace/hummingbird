...
type Encoder interface {
	//编码
	Encode(content string, ecl ErrorCorrectionLevel) (*BitList, *Version, error)
}
...
//获取编码类型对应的编码器
func (e EncoderType) getEncoder() Encoder {
	switch e {
	case Numeric:
	...
	case EightBitByte:
		return EightBitByteEncoder{}
	}
	return UnknowEncoder{}
}

//通过内容获取编码类型
func getEncoderType(content string) EncoderType {
	//numberic
	numericMatch, _ := regexp.Compile(NumericRegExpType)
	if numericMatch.MatchString(content) {
		return Numeric
	}
	//alphanumberic
	alphaNumericMatch, _ := regexp.Compile(AlphaNumericRegExpType)
	if alphaNumericMatch.MatchString(content) {
		return Alphanumeric
	}
	//其他为unicode
	return Unicode
...
//
func render(data []byte, vi *Version) *QRCode {
	dim := vi.modulWidth()
	results := make([]*QRCode, 8)
	for i := 0; i < 8; i++ {
		results[i] = newQrcode(dim)
	}

	occupied := newQrcode(dim)
...
func drawFinderPatterns(vi *Version, set func(int, int, bool)) {
	dim := vi.modulWidth()
...
func drawAlignmentPatterns(occupied *QRCode, vi *Version, set func(int, int, bool)) {
	drawPattern := func(xoff int, yoff int) {
		for x := -2; x <= 2; x++ {
			for y := -2; y <= 2; y++ {
...
func drawFormatInfo(vi *Version, usedMask int, set func(int, int, bool)) {
	var formatInfo []bool
...
var VersionBitsByVersion = map[byte][]bool{
...
func drawVersion(vi *Version, set func(int, int, bool)) {
	VersionBits, ok := VersionBitsByVersion[vi.Version]

	if ok && len(VersionBits) > 0 {
		for i := 0; i < len(VersionBits); i++ {
			x := (vi.modulWidth() - 11) + i%3
			y := i / 3
			set(x, y, VersionBits[len(VersionBits)-i-1])
			set(y, x, VersionBits[len(VersionBits)-i-1])
		}
	}
...
//添加补充位和结束符
func addPaddingAndTerminator(bl *BitList, v *Version) {
	for i := 0; i < 4 && bl.Len() < v.getNumberOfDataCodewords()*8; i++ {
		bl.AddBit(false)
	}
	...
	for i := 0; bl.Len() < v.getNumberOfDataCodewords()*8; i++ {
		if i%2 == 0 {
			bl.AddByte(236)
		} else {
			bl.AddByte(17)
...
func Encode(content string, level ErrorCorrectionLevel, mode EncoderType) (BarCode, error) {
	bits, vi, err := mode.getEncoder().Encode(content, level)
	if err != nil {
		fmt.Println("ERROR Encode mode.getEncoder().Encode error:", err)
		return nil, err
	}
	fmt.Println("Encode: ", "EncodeType=", mode, ", Version=", int(vi.Version), " ,Level=", int(vi.Level))
...
// 此处只需要QRCode中的dimension和content
// 导出png图,默认L
func EncodeToPng(qrCode *QRCode, w io.Writer) error {
	if len([]rune(qrCode.content)) == 0 {
		return ArguementsNullError
	}
	//自动选择合适的Version
	encodeType := getEncoderType(qrCode.content)
	startTime := time.Now()
	if barCode, err := Encode(qrCode.content, L, encodeType); err == nil {
		if err := ToPng(barCode, qrCode.Dimension(), w); err != nil {
			return PNGEncodeError
		}
	} else {
		return err
	}
	fmt.Println("EncodeToPng used: ", util.CostTimeCalc(startTime, time.Now()))
	return nil
...
func ToImage(qrcode BarCode, dimension int) (image.Image, error) {
	...
	var img image.Image
	if destImgSize != nil && destImgSize.width != dimension {
		img_ := image.NewRGBA(image.Rect(0, 0, dimension, dimension))
		//缩放到dimension大小
		if err = myimage.Scale(img_, qrcode_); err != nil {
			fmt.Print("ERR ToImage Scale error:", err)
			return nil, err
		}
		img = img_
	} else {
